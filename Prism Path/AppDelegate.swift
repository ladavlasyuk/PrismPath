...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private var stagedColdLaunchNotificationID: String?
    private var pendingColdPushForLaunch: String?
    private var didStartLaunchFlow = false
    private var coldLaunchPushHandled = false
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        LaunchState.shared.clearPersistedPushDestination()
    ...
    func application(
        ...
        if let response = options.notificationResponse {
            stagedColdLaunchNotificationID = response.notification.request.identifier
            stageColdLaunchPush(userInfo: response.notification.request.content.userInfo)
        }
    ...
    private func stageColdLaunchPush(userInfo: [AnyHashable: Any]) {
        ...
        pendingColdPushForLaunch = addressString
        consumedLaunchPushAddress = addressString
        print("[Push] Cold launch push staged: \(addressString)")
    }

    @discardableResult
    func attachWindow(to windowScene: UIWindowScene) -> UIWindow {
        ...
        newWindow.rootViewController = LaunchLoaderViewController()
    ...
    func handleSceneNotificationResponse(_ response: UNNotificationResponse) {
        ...
        logPushPayload(
            content.userInfo,
            source: "sceneConnect (notificationResponse)",
            content: content,
            extraLines: [
        ...
        if LaunchState.shared.isPermanentNativeFlow() {
            print("[Push] Scene tap ignored — permanent native flow")
            return
        }

        if notificationId == stagedColdLaunchNotificationID {
            print("[Push] Cold launch push already staged — skipping duplicate navigation")
            return
        }
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse
                ...
                self.logTiming("No server response within timeout — opening last successful link if any")
                self.openLastSuccessfulLinkOrOffline()
            }
    ...
    private func updateLaunchPresentation() {
        ...
        if isLaunchLoaderActive {
            if state.noInternetMessage != nil {
                logTiming("Launch decision: server unreachable — showing no internet")
            ...
            if let destination = state.browserDestination {
                ...
                switchRootToBrowser(
                    ...
                    forceReload: LaunchState.shared.consumeForceBrowserReload()
                )
                return
            }
            ...
            if didReceiveConfigResponse {
                logTiming("Launch decision: server answered without a link — switching loader to native")
                isLaunchLoaderActive = false
                cancelLoaderSafetyTimeout()
        ...
        if isBrowserPresented(in: window) || rootBrowser != nil {
            OrientationController.shared.unlockAllOrientations()
            if state.noInternetMessage != nil {
                visibleBrowserController()?.showOfflinePlaque()
            } else if let destination = state.browserDestination {
                presentBrowserDestination(destination, forceReload: LaunchState.shared.consumeForceBrowserReload())
            }
            return
        }
    ...
    private func switchRootToBrowser(window: UIWindow, destination: String, forceReload: Bool = false) {
        OrientationController.shared.unlockAllOrientations()
        ...
        if let existing = rootBrowser {
            if (forceReload || existing.destination != destination), let address = URL(string: destination) {
                ...
                if forceReload {
                    existing.loadAsNewRoot(address: address)
                } else {
    ...
    private func presentBrowserDestination(_ destination: String, forceReload: Bool = false) {
        ...
        if window.rootViewController?.presentedViewController != nil {
            window.rootViewController?.dismiss(animated: false) { [weak self] in
                ...
                self.switchRootToBrowser(window: window, destination: destination, forceReload: forceReload)
            }
        } else {
            switchRootToBrowser(window: window, destination: destination, forceReload: forceReload)
        }
    }
    ...
    private func continueLaunchFlow() {
        ...
        if let coldPushAddress = pendingColdPushForLaunch {
            pendingColdPushForLaunch = nil
            consumedLaunchPushAddress = nil
            coldLaunchPushHandled = true
            LaunchState.shared.beginPushSessionLock(coldPushAddress)
            LaunchState.shared.prepareToOpenBrowser(coldPushAddress)
            refreshConfigInBackgroundIfNeeded()
            return
        }
        ...
        if LaunchState.shared.activateStoredDestinationIfValid() {
            print("[AFSDK] Stored link is still valid — opening it without a config request.")
            expectsBrowserDestination = true
            didReceiveConfigResponse = true
            OrientationController.shared.unlockAllOrientations()
            updateLaunchPresentation()
            return
        ...
        if let storedPayload = LaunchState.shared.storedConfigPayload() {
            print("[AFSDK] No usable destination yet. Reusing saved payload for config request.")
    ...
    private func openStoredConfigDestination() {
        guard !LaunchState.shared.isPermanentNativeFlow() else { return }
        if LaunchState.shared.activateStoredDestinationIfValid() {
            return
        }
        if let destination = LaunchState.shared.lastSuccessfulDestination() {
            LaunchState.shared.prepareToOpenBrowser(destination)
        }
    }
    ...
    private func handleRemoteNotificationUserInfo(_ userInfo: [AnyHashable: Any], fromUserTap: Bool) {
        ...
        guard !LaunchState.shared.isPermanentNativeFlow() else {
            print("[Push] Tap ignored — permanent native flow")
            return
        }

        guard let addressString = extractPushAddress(from: userInfo) else {
            print("[Push] Tap has no address — opening stored config link")
            DispatchQueue.main.async { [weak self] in
                self?.openStoredConfigDestination()
            }
            return
        }

        print("[Push] Opening push address: \(addressString)")
        DispatchQueue.main.async {
            LaunchState.shared.handleIncomingPushAddress(addressString)
        }
    }

    private func openLastSuccessfulLinkOrOffline() {
        if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
            print("[AFSDK] Endpoint did not answer — opening last successful link.")
            didReceiveConfigResponse = true
            expectsBrowserDestination = true
            LaunchState.shared.clearNoInternetMessage()
            updateLaunchPresentation()
        ...
        print("[AFSDK] Endpoint did not answer and there is no saved link.")
    ...
    private func handleLaunchPushNotification(_ userInfo: [AnyHashable: Any]) {
        ...
        stageColdLaunchPush(userInfo: userInfo)
    }

    private func refreshConfigInBackgroundIfNeeded() {
        if !LaunchState.shared.isStoredDestinationExpired() {
            print("[AFSDK] Stored link is still valid — skipping config request.")
            return
        }
        if let storedPayload = LaunchState.shared.storedConfigPayload() {
            print("[AFSDK] Refreshing config in background without replacing active session destination.")
    ...
    private func concludeLaunchWithoutRemoteContent() {
        LaunchState.shared.clearNoInternetMessage()
        didReceiveConfigResponse = true
        expectsBrowserDestination = false
        updateLaunchPresentation()
    ...
    private func parsedConfigPayload(from data: Data?) -> [String: Any]? {
        guard let data,
              !data.isEmpty,
    ...
    private func sendMergedPayload(_ payload: [String: Any], applyDestinationToCurrentSession: Bool = true) {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let error = error {
                ...
                DispatchQueue.main.async {
                    let keepCurrentSession = !applyDestinationToCurrentSession
                        && (LaunchState.shared.isPushNavigationActive()
                            || LaunchState.shared.browserDestination != nil
                            || LaunchState.shared.pendingDestination != nil)
                    if keepCurrentSession {
                        print("[AFSDK] Config request failed — keeping the address already on screen.")
                        self?.didReceiveConfigResponse = true
                        self?.expectsBrowserDestination = true
                        self?.updateLaunchPresentation()
                        return
                    }
                    print("[AFSDK] Config request failed — server unreachable.")
                    self?.openLastSuccessfulLinkOrOffline()
                }
                return
            }
            ...
            if !(200...299).contains(status) {
                print("[AFSDK] Config HTTP \(status)")
                DispatchQueue.main.async {
                    LaunchState.shared.clearNoInternetMessage()
                    if self?.parsedConfigPayload(from: data) != nil {
                    ...
                    print("[AFSDK] Config HTTP \(status) without a link — opening last successful link if any.")
                    self?.openLastSuccessfulLinkOrOffline()
                }
                return
            }
    ...
    private func handleMergedPayloadResponse(
        ...
        guard let data = data,
              let json = try? JSONSerialization.jsonObject(with: data),
              let payload = json as? [String: Any] else {
            print("[AFSDK] Server response is empty or invalid — opening last successful link if any.")
            if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
                return true
            }
            LaunchState.shared.clearNoInternetMessage()
        ...
        guard hasOK else {
            print("[AFSDK] Server returned ok=false — native flow.")
            LaunchState.shared.clearNoInternetMessage()
            return false
        }

        guard let destination = destination else {
            print("[AFSDK] Server returned ok=true without a destination — native flow.")
            LaunchState.shared.clearNoInternetMessage()
            return false
        }
    ...
    private func visibleBrowserController() -> ContentBrowserController? {
        if let browser = rootBrowser {
            return browser
        }
        guard var top = window?.rootViewController else { return nil }
        if let browser = top as? ContentBrowserController {
            return browser
        }
        while let presented = top.presentedViewController {
            if let browser = presented as? ContentBrowserController {
                return browser
            }
            top = presented
        }
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if notificationId == stagedColdLaunchNotificationID {
            print("[Push] Cold launch push staged — skipping didReceive duplicate")
            completionHandler()
            return
        }
