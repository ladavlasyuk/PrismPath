...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func handleSceneNotificationResponse(_ response: UNNotificationResponse) {
        ...
        if isWebContentAlreadyPresented() {
            print("[Push] Web session already open — keeping current address")
            return
        }

        if let addressString = extractPushAddress(from: content.userInfo),
           pendingColdPushForLaunch == addressString || consumedLaunchPushAddress == addressString {
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse
                ...
                self.logTiming("No server response within timeout — showing no internet")
    ...
    private func updateLaunchPresentation() {
        ...
        if isBrowserPresented(in: window) || rootBrowser != nil {
            ...
            if state.noInternetMessage != nil {
                ...
                presentBrowserDestination(destination, forceReload: false)
            }
            return
        }
    ...
    private func isWebContentAlreadyPresented() -> Bool {
        if rootBrowser != nil || isBrowserPresented(in: window) {
            return true
        }
        if LaunchState.shared.browserDestination != nil
            || LaunchState.shared.pendingDestination != nil
            || LaunchState.shared.isPrePermissionVisible {
            return true
        }
        return false
    }
    ...
    private func handleRemoteNotificationUserInfo(_ userInfo: [AnyHashable: Any], fromUserTap: Bool) {
        ...
        if coldLaunchPushHandled {
            print("[Push] Cold launch push navigation already handled — skipping duplicate tap")
            return
        }
        ...
        print("[Push] Cold-launch tap — opening push address: \(addressString)")
        DispatchQueue.main.async {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if !(200...299).contains(status) {
                ...
                DispatchQueue.main.async {
                    ...
                    print("[AFSDK] Server answered without a link — native flow.")
                    self?.didReceiveConfigResponse = true
                    self?.expectsBrowserDestination = false
                    self?.updateLaunchPresentation()
                }
    ...
    private func handleMergedPayloadResponse(
        ...
        guard let data = data,
              let json = try? JSONSerialization.jsonObject(with: data),
              let payload = json as? [String: Any] else {
            print("[AFSDK] Server response is empty or invalid — native flow.")
            LaunchState.shared.clearNoInternetMessage()
            return false
        }
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if coldLaunchPushHandled {
            print("[Push] Cold launch push already handled — skipping didReceive duplicate")
            completionHandler()
            return
        }
