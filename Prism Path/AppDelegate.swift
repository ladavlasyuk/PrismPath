...
class AppDelegate: UIResponder, UIApplicationDelegate {
        ...
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse
                ...
                if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
                    self.logTiming("No server response within timeout — opening last successful link")
                } else {
    ...
    private func updateLaunchPresentation() {
        ...
        if isLaunchLoaderActive {
            ...
            if didReceiveConfigResponse {
                if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
                    logTiming("Launch decision: config failed — opening last successful link")
                    isLaunchLoaderActive = false
                    cancelLoaderSafetyTimeout()
                    OrientationController.shared.unlockAllOrientations()
    ...
    private func continueLaunchFlow() {
        ...
        if LaunchState.shared.activateStoredDestinationIfValid() {
            ...
            if let storedPayload = LaunchState.shared.storedConfigPayload(),
               LaunchState.shared.isStoredDestinationExpired() {
                print("[AFSDK] Stored destination still usable but expired — refreshing config in background.")
                sendMergedPayload(storedPayload, applyDestinationToCurrentSession: true)
            }
            return
        }

        if let lastDestination = LaunchState.shared.lastSuccessfulDestination() {
            OrientationController.shared.unlockAllOrientations()
            print("[AFSDK] Opening last successful destination while refreshing config: \(lastDestination)")
            _ = LaunchState.shared.activateLastSuccessfulDestinationAsFallback()
            if let storedPayload = LaunchState.shared.storedConfigPayload() {
                sendMergedPayload(storedPayload, applyDestinationToCurrentSession: true)
            } else {
    ...
    private func handleRemoteNotificationUserInfo(_ userInfo: [AnyHashable: Any], fromUserTap: Bool) {
        ...
        guard LaunchState.shared.hasStoredDestination() || LaunchState.shared.browserDestination != nil || rootBrowser != nil else {
        ...
        DispatchQueue.main.async { [weak self] in
            LaunchState.shared.applySessionDestination(addressString)
            self?.presentBrowserDestination(addressString, forceReload: true)
        }
    }
    ...
    private func sendMergedPayload(_ payload: [String: Any], applyDestinationToCurrentSession: Bool = true) {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let error = error {
                ...
                DispatchQueue.main.async {
                    let openedFallback = LaunchState.shared.activateLastSuccessfulDestinationAsFallback()
                    if openedFallback {
                        print("[AFSDK] Config request failed — keeping last successful link.")
                    } else {
                        print("[AFSDK] Config request failed and no saved link — showing offline plaque.")
                        self?.showNoInternetMessageIfNeeded(error)
                    }
                    self?.didReceiveConfigResponse = true
                    self?.expectsBrowserDestination = openedFallback
                    self?.updateLaunchPresentation()
                }
                return
            }
            ...
            if !(200...299).contains(status) {
                print("[AFSDK] Config HTTP \(status) — treating as unavailable endpoint.")
                DispatchQueue.main.async {
                    let openedFallback = LaunchState.shared.activateLastSuccessfulDestinationAsFallback()
                    if !openedFallback {
                        LaunchState.shared.showNoInternetMessage()
    ...
    private func handleMergedPayloadResponse(
        ...
        guard let data = data,
            ...
            if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
                print("[AFSDK] Using last successful link after invalid config response.")
                return true
            }
            print("[AFSDK] No saved link available after invalid config response.")
            return false
        }
        ...
        guard hasOK else {
            print("[AFSDK] Server returned ok=false.")
            if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
                print("[AFSDK] Keeping last successful link after ok=false.")
                return true
            }
        ...
        guard let destination = destination else {
            ...
            if LaunchState.shared.activateLastSuccessfulDestinationAsFallback() {
                print("[AFSDK] Keeping last successful link after empty destination.")
                return true
            }
            print("[AFSDK] Native flow will remain active.")
            return false
