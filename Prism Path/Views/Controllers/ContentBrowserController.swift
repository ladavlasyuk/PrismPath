...
final class ContentBrowserController: UIViewController {
    ...
    private var offlinePlaque: UIView?
    private var offlineImageView: UIImageView?
    private var pathMonitor: NWPathMonitor?
    private var isOfflinePlaqueVisible = false
    ...
    private func setupOfflinePlaque() {
        let plaque = UIView()
        plaque.translatesAutoresizingMaskIntoConstraints = false
        plaque.backgroundColor = .black
        plaque.isHidden = true
        ...
        plaque.addSubview(imageView)

        NSLayoutConstraint.activate([
            plaque.topAnchor.constraint(equalTo: view.topAnchor),
            plaque.bottomAnchor.constraint(equalTo: view.bottomAnchor),
            plaque.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            plaque.trailingAnchor.constraint(equalTo: view.trailingAnchor),

            imageView.topAnchor.constraint(equalTo: plaque.topAnchor),
            imageView.bottomAnchor.constraint(equalTo: plaque.bottomAnchor),
            imageView.leadingAnchor.constraint(equalTo: plaque.leadingAnchor),
            imageView.trailingAnchor.constraint(equalTo: plaque.trailingAnchor)
        ])

        offlinePlaque = plaque
        offlineImageView = imageView
        updateOfflinePlaqueImage(for: view.bounds.size)
    }

    private func updateOfflinePlaqueImage(for size: CGSize) {
        let name = size.width > size.height ? "NoInternetBackgroundLandscape" : "NoInternetBackground"
        offlineImageView?.image = UIImage(named: name)
    }
        ...
        monitor.pathUpdateHandler = { [weak self] path in
            DispatchQueue.main.async {
                ...
                if path.status == .satisfied {
                    self.hideOfflinePlaque()
                } else {
                    self.showOfflinePlaque()
                }
            }
        }
        monitor.start(queue: DispatchQueue(label: "com.prismpath.contentPath"))
    }

    func showOfflinePlaque() {
        isOfflinePlaqueVisible = true
        hideErrorView()
        updateOfflinePlaqueImage(for: view.bounds.size)
        guard let plaque = offlinePlaque else { return }
        plaque.isHidden = false
        view.bringSubviewToFront(plaque)
    }

    func hideOfflinePlaque() {
        guard isOfflinePlaqueVisible else { return }
        isOfflinePlaqueVisible = false
        offlinePlaque?.isHidden = true
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        updateOfflinePlaqueImage(for: size)
    }
    ...
    private func loadDestination() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 15) { [weak self] in
            ...
            if !self.hasLoadedMainContent && self.contentView.url == nil {
                print("[Browser] Load timed out with no content — showing no internet")
                self.showOfflinePlaque()
            } else {
    ...
    func loadAsNewRoot(address: URL) {
